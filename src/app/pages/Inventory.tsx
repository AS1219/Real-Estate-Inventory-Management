...
import { projects, generateAllFlats, type FlatStatus } from '../data/mockData';

export function Inventory() {
  const [filterStatus, setFilterStatus] = useState<FlatStatus | 'all'>('all');
  const [searchQuery, setSearchQuery] = useState('');
  
  const allFlats = projects.flatMap(project =>
    generateAllFlats(project.id, project.buildings.length).map(flat => ({
      ...flat,
      projectName: project.name,
      buildingName: project.buildings.find(b => b.id === flat.buildingId)?.name || ''
    }))
  );
          ...
          <div className="p-4 md:p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <div className="p-4 bg-[#F9FAFB] rounded-[16px]">
                <div className="text-2xl font-semibold text-[#111827] mb-1">{allFlats.length}</div>
              ...
              <div className="p-4 bg-[#F59E0B]/10 rounded-[16px]">
                <div className="text-2xl font-semibold text-[#F59E0B] mb-1">
                  {allFlats.filter(f => f.status === 'blocked').length}
                </div>
              ...
              <div className="p-4 bg-[#DC2626]/10 rounded-[16px]">
                <div className="text-2xl font-semibold text-[#DC2626] mb-1">
                  {allFlats.filter(f => f.status === 'sold').length}
                </div>
